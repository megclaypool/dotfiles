# Generated by Powerlevel10k configuration wizard on 2022-08-15 at 21:34 PDT.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    # my_fancy_dir              # fancy directory
    # my_home                 # home directory (will display as icon)
    vcs                     # git status
    # =========================[ Line #2 ]=========================
    newline                 # \n
...
function get_my_fancy_dir() {
  if [[ $PWD == *"Users/noxlady"* ]]; then
    temp="⭐"
    temp+=${PWD#'/Users/noxlady'}
  else
    temp=$PWD
  fi
  return $temp
}
  # Replace '~' with house icon in prompt
  function prompt_my_fancy_dir() {
    # if [[ $PWD == *"Users/noxlady"* ]]; then
    #   temp=${PWD#"/Users/noxlady"}
    #   echo ${get_my_fancy_dir}
      # echo $temp
      # p10k segment  -i '⭐' -t $temp
      # p10k segment  -i '⭐' -f blue -t $temp
      # p10k segment  -i '⭐' -f blue -t 'hello'
    # fi
  }
...
# POWERLEVEL9K_VCS_STASHED_ICON=$'\U2605\u200a'
# POWERLEVEL9K_VCS_STASHED_ICON=$'\Uf069\u200a'
POWERLEVEL9K_VCS_STASHED_ICON=$'\Uf041\u200a'
POWERLEVEL9K_VCS_STAGED_ICON=$'\uf055\u200a'
POWERLEVEL9K_VCS_UNSTAGED_ICON=$'\uf056\u200a'
POWERLEVEL9K_VCS_UNTRACKED_ICON=$'\uf00d\u200a'
POWERLEVEL9K_VCS_CONFLICT_ICON=$'\uf421\u200a'
...
# # VCS colours
  ...
  typeset -g POWERLEVEL9K_VCS_CONFLICTED_BACKGROUND='088'
  ...
  # typeset -g POWERLEVEL9K_VCS_MODIFIED_FOREGROUND='white'
    ...
    (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${clean}${(g::)POWERLEVEL_9K_VCS_OUTGOING_CHANGES_ICON}${VCS_STATUS_COMMITS_BEHIND}"
    # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
    (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${clean}${(g::)POWERLEVEL_9K_VCS_OUTGOING_CHANGES_ICON}${VCS_STATUS_COMMITS_AHEAD}"
    # ⇠42 if behind the push remote.
    (( VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" ${clean}⇠${VCS_STATUS_PUSH_COMMITS_BEHIND}"
    (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" "
    # ⇢42 if ahead of the push remote; no leading space if also behind: ⇠42⇢42.
    # (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && res+="${clean}⇢${VCS_STATUS_PUSH_COMMITS_AHEAD}"
    (( VCS_STATUS_PUSH_COMMITS_AHEAD  )) && res+="${clean}${(g::)POWERLEVEL_9K_VCS_OUTGOING_CHANGES_ICON}${VCS_STATUS_PUSH_COMMITS_AHEAD}"
    # *42 if have stashes.
...
#   local failure_color='196'
#   # local success="✔"
#   local success_symbol='\u26A1'
#   local success_color='226'
#   local prompt_background='236'
#   local prompt_arrow='\ue0b0'

#   if [[ "$last_status" != "0" ]]; then
#       last_status="%K{$prompt_background}%F{$failure_color} $failure_symbol%f%k%F{$prompt_background}$prompt_arrow%f "
#   else
#       last_status="%K{$prompt_background}%F{$success_color} $success_symbol%f%k%F{$prompt_background}$prompt_arrow%f "
#   fi

#   echo "$last_status"
# }

# ##### This is supposed to allow me to change the battery icon if the laptop is plugged in (adding a plug icon) but I'm getting permission errors when I call it in precmd!
...
# precmd() {
#   POWERLEVEL9K_MULTILINE_LAST_PROMPT_PREFIX="$(BashPrompt)"
# #   "$(BatteryStatus)"
# }
...
# POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(root_indicator background_jobs date battery)
...
# POWERLEVEL9K_STATUS_OK_FOREGROUND='226'
...
# POWERLEVEL9K_LINUX_ICON='%F{023} \uF179 %f'
...
# # Battery
# local battery_background='236'
# POWERLEVEL9K_BATTERY_LEVEL_BACKGROUND=(088 202 202 226 226 226 226 029 029 029 029)
# POWERLEVEL9K_BATTERY_LEVEL_FOREGROUND=(white black white black white black white black white black white)
# POWERLEVEL9K_BATTERY_STAGES=($'\uf244 ' $'\uf243 ' $'\uf242 ' $'\uf241 ' $'\uf240 ')
